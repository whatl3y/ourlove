...
export default class RedisHelper {
  ...
  number_of_keys(cb=this.noop) {
    this.dbsize(cb)
  }

  type(key,cb=this.noop) {
    this.client.type(key,cb)
  }

  scan(cursor,cb=this.noop) {
    this.client.scan(cursor,cb)
  }

  expire(key, timeoutSeconds, callback=this.noop) {
  ...
  // wildcard is *
  keys(pattern, callback=this.noop) {
    return new Promise((resolve, reject) => {
  ...
  delete_keys_like(pattern, cb=this.noop) {
    return new Promise((resolve, reject) => {
  ...
  getAndDelete(key, callback=this.noop) {
    this.client
    .multi()
    .get(key)
    .del(key)
  ...
  setnx(key, value, callback=this.noop) {
    return new Promise((resolve, reject) => {
      async_waterfall([
        _callback => this.parseData(value, true, _callback),
  ...
  setnxex(key, value, timeoutSeconds, callback=this.noop) {
    return new Promise((resolve, reject) => {
      async_waterfall([
        _callback => this.parseData(value, true, _callback),
  ...
  hgetall(key, cb=this.noop) {
    this.client.hgetall(key, cb)
  }

  hashgetall(key, cb=this.noop) {
    this.hgetall(key, cb)
  }
  ...
  setgetall(key, cb=this.noop) {
    this.client.smembers(key,cb)
  }

  async zall(key, cb=this.noop) {
    try {
  ...
  zadd(key, value, score=new Date().valueOf(), callback=this.noop) {
    this.client.zadd(key, score, value, callback)
  }

  zsetadd(key, value, score=new Date().valueOf(), callback=this.noop) {
    return new Promise((resolve, reject) => {
